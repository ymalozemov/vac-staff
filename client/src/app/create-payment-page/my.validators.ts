...
export class MyValidators {
  ...
  static formValidador(form: FormGroup) {
    if ((form.get('applicant').value || 0) != (form.get('barcodesRus').value.length + form.get('barcodesGratis').value.length + form.get('barcodesFullGratis').value.length + form.get('barcodesNonRus').value.length)) {
